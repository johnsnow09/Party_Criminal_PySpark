...
spark = SparkSession.builder.appName("Pyspark_Election_Analysis").getOrCreate()
...
@st.cache_resource
def get_data():
    df = spark.read.option("header",True).parquet(r'V:\1. R & Python work\Python\2.AnalytixLabs Practice\Election App Pyspark Streamlit\Elections_Data_Compiled.parquet')
...
@st.cache_resource
def get_shape_data():
    final_shp_export = gp.read_file(r'V:\1. R & Python work\Python\2.AnalytixLabs Practice\Election App Pyspark Streamlit\shape_file_exported\final_shp_export.shp').rename(
...
def get_data_top_parties():
    return spark.read.option("header",True).csv(r'V:\1. R & Python work\Python\2.AnalytixLabs Practice\Election App Pyspark Streamlit\For_Analysis_in_R.csv')
...
with header_mid:
    ...
    st.caption("<p style='text-align: center; font-size: 20px;'><b>PySpark</b> is used along with Python for data processing</p>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("<p style='text-align: center; font-size: 20px;'><b>PySpark</b> App</p>", unsafe_allow_html=True)
    ...
    def get_state_list():
        ...
        return df.select('State').distinct().toPandas().State.tolist()
...
############################## SECOND FILTER YEAR ##############################
    ...
    def get_year_list(State_Selected):
        ...
        return (df.filter(F.col('State') == State_Selected)
                .select(F.col('Year')).distinct().sort(F.desc('Year')).toPandas()['Year'].tolist())
...
############################## SIDEBAR DISCAILMER ##############################
    ...
    st.write("*Disclaimer:* - This app is for **educational** Purpose only and to demonstrate the use of **PySpark & Python**  \n   \
             \n We do not guarantee for Authenticity of the data and would request you to do your own research.  \
             \n It uses very small portion of data from https://myneta.info/ and aggregates using **PySpark** to handle **data processing** efficiently.   \n \
...
df_selected = df.filter((F.col('State') == State_Selected) &
                    (F.col('Year').isin(Year_Selected))
                    )
...
cases_agg_2022 = (df_selected.groupBy(['Party'])
                .agg((F.count('Party').alias('candidates_count')),
                    (F.sum('Criminal_Case').alias('total_criminal_cases')))
                .withColumn('avg_cases',(F.col('total_criminal_cases') / F.col('candidates_count') ))
                .sort(F.desc('total_criminal_cases'))
                )


Major_Parties = (
    cases_agg_2022
    .filter(F.col('total_criminal_cases')>0)
    .sort(F.desc('total_criminal_cases'))
    .limit(6)
    .select(F.col('Party'))
    .toPandas()['Party']
...
highest_criminal_parties = df_selected.groupBy(['Party']
                                ).agg(F.sum('Criminal_Case').alias('Criminal_Case')
                                ).sort(F.desc('Criminal_Case'))

highest_criminal_party = highest_criminal_parties.select(F.col('Party')).limit(1).toPandas()['Party'].tolist()
# highest_criminal_party6 = highest_criminal_parties.select(pl.col('Party')).head(6).to_series().to_list()

with plt_box_1:

    fig_party_crime_sum = px.bar(highest_criminal_parties.limit(18).toPandas(),
                                orientation='h',
...
with boxplt_2:
    fig_party_crime_box = px.box(df_selected.filter(
            (F.col('Party').isin(Major_Parties)) # highest_criminal_parties.select(pl.col('Party')).head(6).to_series().to_list()
                                    ).toPandas(),
            x = 'Party',
            y = 'Criminal_Case',
            color= 'Party',
...
with plt3_box_1:
    fig_cases_count_party_facet = px.bar(df_selected.filter(F.col('Party').isin(Major_Parties) & 
                                        (F.col('Criminal_Case') > 3)).sort(F.desc('Criminal_Case')
                                        ).groupBy(
                                        ['Party','Criminal_Case']).count().toPandas(),
                                        orientation='h',
...
fig_party_cand_count = px.bar(cases_agg_2022.sort(F.desc('candidates_count')
                                ).limit(18).toPandas(),
                                orientation='h',
...
fig_party_avg_cases = px.bar(cases_agg_2022.sort(F.desc('avg_cases')
                                ).limit(18).toPandas(),
                                orientation='h',
...
# converting above code to facet year
    fig_party_asset_sum = px.bar(df.filter(F.col('State') == State_Selected).groupBy(['Party','Year']
                                    ).agg((F.sum('Total_Assets')/10**7).alias('Total_Assets')
                                    ).sort(F.desc('Total_Assets')
                                    ).limit(18).toPandas(),
                                    orientation='h',
                                    x='Total_Assets',y='Party', color="Party",
...
with asset_2:
    fig_party_asset_buble = px.scatter(df_selected.filter(
                                        (F.col('Party').isin(Major_Parties)) 
                                        ).toPandas(),
                                        x = 'Party',
                                        y = 'Total_Assets',
                                        hover_name='Party',
...
fig_crime_asset_buble = px.scatter(df_selected.filter(F.col('Party').isin(Major_Parties)).toPandas(),
...
fig__edu_crime_buble = px.scatter(df_selected.filter((F.col('Party').isin(Major_Parties)) &
                                                            (F.col('Criminal_Case') > 0)).toPandas(),
...
fig__edu_crime_buble_facet2 = px.scatter(df_selected.filter((F.col('Party').isin(Major_Parties)) &
                                                            (F.col('Criminal_Case') > 0)).toPandas(),
...
fig_const_crime_sum = px.bar(
                                    df_selected.filter(
                                        F.col('Party').isin(Major_Parties)).groupBy(['Constituency','Party']
                                        ).agg(F.sum('Criminal_Case').alias('Criminal_Case')
                                        ).sort(F.desc('Criminal_Case')
                                        ).toPandas()
